...
//#define GPIO_LEDS_STROBE   //lingjinming 
...
int FL_enable(void)
{
   ...
   //lingjinming // upmu_set_isink_ch0_en(0x1); // Turn on ISINK Channel 0                  
...
//lingjinming //upmu_set_isink_ch0_en(0x0);
...
void Flash_Setting_Torch(kal_uint32 torch_mode) ///addd by maxyu 140505
{
	if(1==torch_mode)
	{
	...
	else
	{
	   ...
	   upmu_set_isink_sfstr1_tc(0x0); // 0.5us
	   upmu_set_isink_sfstr1_en(0x0); // Disable soft start
	   upmu_set_rg_isink1_double_en(0x1); // Disable double current
	   upmu_set_isink_phase1_dly_en(0x0); // Disable phase delay
	   upmu_set_isink_chop1_en(0x0); // Disable CHOP clk
	}
}
...
int FL_dim_duty(kal_uint32 duty)
{
	...
	if(duty ==6)	
		{
		Flash_Setting_Torch(1);

		}
	else ///12
		{
		Flash_Setting_Torch(0);
		}
