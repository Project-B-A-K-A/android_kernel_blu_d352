...
void GC0328NightMode(kal_bool bEnable)
{
	...
	else 
	{
		...
		GC0328_write_cmos_sensor(0xfe, 0x00);
		GC0328GammaSelect(GC0328_RGB_Gamma_m6); // m4
...
void GC0328_Sensor_Init(void)
{
	...
	GC0328_write_cmos_sensor(0x24 , 0x16); //0x11 jon   
	...
	GC0328_write_cmos_sensor(0x82 , 0x87);    
	...
	///////YCP
	GC0328_write_cmos_sensor(0xd1 , 0x20);///0x30
	GC0328_write_cmos_sensor(0xd2 , 0x20);///0x30
	GC0328_write_cmos_sensor(0xd3 , 0x38);// 42
	...
	GC0328_write_cmos_sensor(0x13 , 0xb0);    ///  lanking +2  //80
...
{
    GC0328GammaSelect(GC0328_RGB_Gamma_m6);//0:use default  m3
}
...
BOOL GC0328_set_param_exposure(UINT16 para)
{
	switch (para)
	{
		...
		case AE_EV_COMP_00:
			...
			GC0328_write_cmos_sensor(0xfe, 0x01);
			GC0328_write_cmos_sensor(0x13, 0xb0);  //////// lanking  //80
			GC0328_write_cmos_sensor(0xfe, 0x00);
		break;
		...
		case AE_EV_COMP_03:
			...
			GC0328_write_cmos_sensor(0xfe, 0x01);
			GC0328_write_cmos_sensor(0x13, 0xb8);
			GC0328_write_cmos_sensor(0xfe, 0x00);
		break;
		...
		case AE_EV_COMP_10:
			...
			GC0328_write_cmos_sensor(0xfe, 0x01);
			GC0328_write_cmos_sensor(0x13, 0xc8);
			GC0328_write_cmos_sensor(0xfe, 0x00);
		break;
		...
		case AE_EV_COMP_13:
			...
			GC0328_write_cmos_sensor(0xfe, 0x01);
			GC0328_write_cmos_sensor(0x13, 0xd0);  //  lanking
			GC0328_write_cmos_sensor(0xfe, 0x00);
		break;
		default:
		return FALSE;
...
#define FLASH_BV_THRESHOLD 0x60
static void GC0328_FlashTriggerCheck(unsigned int *pFeatureReturnPara32)  //added by maxyu 140606 for auto mode flash
{
	unsigned int NormBr;
	
	GC0328_write_cmos_sensor(0xfe, 0x01);//page 1
	NormBr = GC0328_read_cmos_sensor(0x14);
	GC0328_write_cmos_sensor(0xfe, 0x00);//page 0

	printk("GC0328_FlashTriggerCheck NormBr = [0%x]",NormBr);
...
#define GC0328_TEST_PATTERN_CHECKSUM 0x465d5311 // 

UINT32 GC0328SetTestPatternMode(kal_bool bEnable)
{
    printk("[GC0328SetTestPatternMode] Test pattern enable:%d\n", bEnable);
	if(bEnable)    // enable test pattern output  0809 ok!!!
	{
	 ...
	 GC0328_write_cmos_sensor(0xdd, 0x00); 
        ...
        GC0328_write_cmos_sensor(0x4c, 0x01); // 0x11
        ...
        UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
    ...
    case SENSOR_FEATURE_GET_TRIGGER_FLASHLIGHT_INFO:
            GC0328_FlashTriggerCheck(pFeatureData32); //added by maxyu 140606 for auto mode flash
            //SENSORDB("[GC0328] F_GET_TRIGGER_FLASHLIGHT_INFO: %d\n", pFeatureData32);
            break;
