...
#ifdef FTS_GESTRUE
#define TPD_GET_GESTURE_X _IO(TOUCH_IOC_MAGIC,2)
#define TPD_GET_GESTURE_Y _IO(TOUCH_IOC_MAGIC,3)
#endif
	...
	switch(cmd)
	{
	   ...
	   case TPD_GET_VELOCITY_CUSTOM_Y:
		...
		case TPD_GET_GESTURE_X:
			...
			if(copy_to_user(data, &coordinate_x, pointnum*2))
			{
				err = -EFAULT;
				break;
			}
	   ...
	   case TPD_GET_GESTURE_Y:
			...
			if(copy_to_user(data, &coordinate_y, pointnum*2))
			{
				err = -EFAULT;
				break;
			}
...
int check_direct(void)
{
		...
		min_x = max_x = coordinate_x[0];
		min_y = max_y = coordinate_y[0];
		for(i = 1;i < pointnum;i++)
	    	{
			if( coordinate_x[i] < min_x )
				min_x = coordinate_x[i];
			if( coordinate_x[i] > max_x)
				max_x = coordinate_x[i];
			if( coordinate_y[i] < min_y )
				min_y = coordinate_y[i];
			if( coordinate_y[i] > max_y)
				max_y = coordinate_y[i];
		}

		if( 3*( max_y - min_y ) > 4*( max_x - min_x ) )
		{
			
			if( max_x - min_x > 120 )
				return 0;
			if( coordinate_y[pointnum-1] > coordinate_y[0] )
				return GESTURE_DOWN;
			else
				return GESTURE_UP;
		}
		
		else if( 4*( max_y - min_y ) < 3*( max_x - min_x ) )
		{
			if( max_y - min_y > 120 )
				return 0;
			if( coordinate_x[pointnum-1] > coordinate_x[0] )
...
static int ft5x0x_read_Touchdata(void)
{
   ...
   gestrue_id = fetch_object_sample(buf, pointnum);
	...
	if( !gestrue_id && pointnum > 5 )
		gestrue_id = check_direct();

	check_gesture(gestrue_id);
	//zlb add	
	if(!gestrue_id && pointnum >0) 
	{
		for(i =0; i<pointnum; i++)
		{
			tpd_down(coordinate_x[i], coordinate_y[i], 0);
			input_sync(tpd->dev);
			tpd_up(coordinate_x[i], coordinate_y[i]);
			input_sync(tpd->dev);
					// printk("zlb x[%d]=%d,y[%d]=%d,\n",i,cinfo.x[i], i,cinfo.y[i]);
		}
