...
#define TP_PROXIMITY_SENSOR ///added by maxyu 140521
#ifdef TP_PROXIMITY_SENSOR
unsigned char proximity_flag = 0;
unsigned char proximity_state = 0;
extern volatile int call_status;			

int set_proximity_mode(unsigned char mode);
#endif
...
#ifdef TP_PROXIMITY_SENSOR
	if((data[1] == 0xc0) &&(proximity_flag==1)) //close to
		proximity_state=1; 
	else if((data[1] == 0xe0) &&(proximity_flag==1))//leave
		proximity_state=0; 
	
	//printk("tpd_touchinfo-->call_status = %d  \n", call_status);
...
//	if(point_num == 0) return false;
	   ...
	   //TPD_DEBUG("Procss raw data...\n");
		...
		///printk(" cinfo->x[0] = %d, cinfo->y[0] = %d, cinfo->p[0] = %d\n", cinfo->x[0], cinfo->y[0], cinfo->p[0]); //lingjinming add
 ...
 static int touch_event_handler(void *unused)
 {
	...
	hwPowerDown(TPD_POWER_SOURCE_CUSTOM_BAK, "TP");
#endif
...
int set_proximity_mode(unsigned char mode)
{
	unsigned char proximity_mode = mode;

	if(tpd_halt)
		return 0;

	//mutex_lock(&i2c_access);
	i2c_smbus_write_i2c_block_data(i2c_client, 0xB0, 1, &proximity_mode);  
	//mutex_unlock(&i2c_access);

	printk("set_proximity_mode = %d  \n", proximity_mode);
...
int set_proximity_mode_int(unsigned char mode)
{
	unsigned char proximity_mode = mode;
...
int set_proximity_keymode(void) ///added by maxyu 131101
{
	if(!call_status)
		return;
	...
	tpd_up(0,0,0);
	input_sync(tpd->dev);
	TPD_DMESG("set_proximity_keymode done\n");

	if(proximity_state)///close to
	{
		set_proximity_mode_int(0);
		proximity_state=0;///leave
	}
	else
	{
		set_proximity_mode_int(1);
		//proximity_state=1;///close to
...
int set_proximity_keymode(void)
{
